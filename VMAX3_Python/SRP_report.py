...
def createReports(sgObj,site,datacenter):
    ...
    heading = ['Pool','Capacity','Used','Percent Used','Free','Technology','Protection']
      ...
      for pool in sgObj.pools.keys():
        ...
        if tier in comparepool:
          ...
          if int(percent_used) >= alertLimits[tier]:
            ...
            alerts += 1
          row = [pool,str(usable),str(used),str(percent_used),str(free),tech,protection]
          csvpage.append(', '.join(row) + '\n')
          
          page.append(www.tr_list(row))
          
    total_pct_used = int((int(total_used) / (float(total_usable) + .0001)) * 100)
    total_subscription = int((int(total_provisioned) / (float(total_usable) + .0001)) * 100)
    row = ['Totals',str(total_usable),str(total_used),str(total_pct_used),str(total_free),'___','___']
    
    page.append(www.tr_list(row))  
    page.append(www.end_table)

    page.append(www.start_table(3,'SRP Info'))
    heading = ['Name','Capacity','Used','Percent Used','Free','Provisioned','Subscription']
    page.append(www.th_list(heading))
    csvpage.append("\n\n\n")
    csvpage.append(', '.join(heading) + '\n')
    ...
    row = [sgObj.SRP_NAME,str(usable),str(used),str(pct_used),str(free),str(subscribed),str(subscription)]
    csvpage.append(', '.join(row) + '\n')
    page.append(www.tr_list(row))  
    page.append(www.end_table)
