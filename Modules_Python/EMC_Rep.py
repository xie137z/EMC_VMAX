...
class VMAX_STORAGE:
  ...
  def get_fast_demand(self):
    SYMCLI = '/opt/emc/SYMCLI/bin/'
 
    cmd = SYMCLI + 'symfast -sid ' + self.sid  + ' list -demand -association -output xml_e'

    # Exec the sym command...
    output = os.popen(cmd)

    # Write the XML to an XML file so that it can be passed to the parser
    XML = '/var/www/html/XML/' + self.sid + '_fast_demand.xml'
    f = open(XML, 'w')
    for line in output:
      #line = line.strip()
      f.write(line)
    ...
    for node in doc.getElementsByTagName('Fast_Policy'):
      sgName = ""
      fastPolicy = ""
      
      for policy in node.getElementsByTagName('Policy_Info'):
        for child in policy.childNodes:
          if child.nodeType == 1:
            if child.nodeName == "storage_group_name":
      ...
      if sgName not in self.tiers:
        self.tiers[sgName] = {}
        self.tiers[sgName]['SG_USED'] = 0
      for tier in node.getElementsByTagName('Tier'):
        for child in tier.childNodes:
          if child.nodeType == 1:
            if child.nodeName == "tier_name":
              tier_name = child.firstChild.nodeValue
            if child.nodeName == "tier_sg_used":
              tier_used = str(child.firstChild.nodeValue)          
            if child.nodeName == "tier_sg_used_units":
              units = str(child.firstChild.nodeValue)
              # Units is the last bit of data needed. Add this tier entry to the data structure.
              self.tiers[sgName][tier_name] = tier_used + ' ' + units
              self.tiers[sgName]['SG_USED'] += int(tier_used)
              # Keep a running list of tiers for reporting later.
              if tier_name not in self.tiernames:
                self.tiernames.append(tier_name)
        
    
    
  def get_fast(self):
    SYMCLI = '/opt/emc/SYMCLI/bin/'
    cmd = SYMCLI + 'symfast -sid ' + self.sid  + ' list -fp -v -output xml_e'
    ...
    for node in doc.getElementsByTagName('Fast_Policy'):
      fast_tiers = {}
      for info in node.getElementsByTagName('Policy_Info'):
      ...
      for tier in node.getElementsByTagName('Tier'):
        for child in tier.childNodes:
          if child.nodeType == 1:
            if child.nodeName == "tier_name":
              tiername = child.firstChild.nodeValue
            if child.nodeName == "tier_max_sg_per":
              maxpercent = str(child.firstChild.nodeValue)
        fast_tiers[str(tiername)] = str(maxpercent)
      self.fast_policies[str(policy)] = fast_tiers.copy()         
  ...
  def get_fa_info(self):
    SYMCLI = '/opt/emc/SYMCLI/bin/'
    cmd = SYMCLI + 'symcfg -sid ' + self.sid  + ' list -FA all -output xml_e'
    ...
    cmd = SYMCLI + 'symcfg -sid ' + self.sid  + ' list -dir all -address -output xml_e'
